...
md"
---
##### 3. *Expressions* 和 *built-in* Functions
###### 3.0 为什么是表达式

###### 提问：1+2+3是如何计算的？
"

# ╔═╡ 7f6f765e-9875-408d-9d1e-7b2ccff114c3
md"
###### 3.1 基本表达式: Numerals
"
...
md"
###### [*Operator associativity*](https://en.wikipedia.org/wiki/Operator_associativity): *left*-associativity of operator '-' （左结合性）
"
...
# ╔═╡ 53199298-f4df-4054-8b93-37b18804b44e
...
# ╔═╡ b6ce8e9c-33e1-47f1-9d2a-462c00ddab5a
*(1.0,2)

# ╔═╡ b3af6b38-3b06-4966-8e88-3da2fe720e3e
md"
---
###### 3.2.2 *Nested* Combinations（嵌套组合）
...
# ╔═╡ 8399ee71-51a5-4ed2-ae35-e6aafd56195c
md"
###### 为什么要用树？
...
begin
	...
	#-----------------------------------------------------------------------------
	plot(size=(600, 300), xlim=(0, 20), ylim=(3.4, 4.2), legend=:false,ticks=:none, titlefontsize=12)
...
# ╔═╡ fff05e13-597b-4607-bc6e-348451259f40
md"
###### 其他类型：复数

更多内容可参见[中文文档](https://cn.julialang.org/JuliaZH.jl/latest/manual/complex-and-rational-numbers/)
"

# ╔═╡ 486a7e39-b841-4654-a1f0-e70739d63086
1+1im + 2+2im

# ╔═╡ 3053ecc9-da10-47a0-8c44-fda11e08e15e
md"
###### 拓展：广义的“数”与表达式
...
# ╔═╡ 505a31e7-20c1-4d43-96b6-65f23561943c
md"
---
##### 拓展：小型解释器实现1+2+3
...
# ╔═╡ 4e9c013c-a9f2-4afa-b3aa-25aeb49c468e
function infix_to_postfix(tokens::Vector{String})
    precedence = Dict('+' => 1, '-' => 1, '*' => 2, '/' => 2)
    output = String[]
    op_stack = Char[]

    for token in tokens
        if all(isdigit, token) || '.' in token
            push!(output, token)
        elseif token in ["+", "-", "*", "/"]
            while !isempty(op_stack) && last(op_stack) != '(' &&
                  (precedence[last(op_stack)] >= precedence[token[1]])
                push!(output, string(pop!(op_stack)))
            end
        ...
        elseif token == ")"
            ...
            pop!(op_stack)  # Remove the '(' from the stack
        else
...
# ╔═╡ 4f5b09a9-362b-4edf-93d9-a8b2bc7fd5b6
function postfix_to_prefix(postfix::Vector{String})
    stack = String[]
    ...
    return isempty(stack) ? "0" : last(stack)
end

# ╔═╡ 459df1f7-df02-48e9-a8e4-4584d69b5aa6
function evaluate_prefix(prefix::String)
    function apply_operator(operands::Vector{Float64}, op::Char)
        b = pop!(operands)
        a = pop!(operands)
        if op == '+' return a + b
    ...
    for token in reverse(tokens)
        ...
        elseif token in ["+", "-", "*", "/"]
            push!(stack, apply_operator(stack, token[1]))
        else
            error("Invalid token: $token")
        end
    end

    return isempty(stack) ? 0.0 : last(stack)
end

# 测试代码

# ╔═╡ 911ccdd1-c61d-4448-96fd-71965890892d
begin
	infix_tokens = split("( 1 + 2 ) * 3")
	infix_tokens = map(String, infix_tokens)
end

# ╔═╡ 01cec3e4-b1d1-484f-a03e-f7766bdae405
println("Infix expression: ", join(infix_tokens, " "))

# ╔═╡ 4e4d38f6-3dd3-4b8e-9413-39d6f412f76b
postfix_tokens = infix_to_postfix(infix_tokens)

# ╔═╡ ac59784d-f2a4-4723-b387-e0661d7287b6
println("Postfix expression: ", join(postfix_tokens, " "))

# ╔═╡ 1698ed15-a1d4-450a-8b83-b149df5361e7
prefix_expression = postfix_to_prefix(postfix_tokens)

# ╔═╡ 44bdaa83-93e4-4ae3-b301-bd3ce0162824
println("Prefix expression: ", prefix_expression)

# ╔═╡ 5933f632-7ef4-47d9-a2db-beae0078b7c7
result = evaluate_prefix(prefix_expression)

# ╔═╡ 87d1d230-4daf-493c-bc43-576ff2179817
